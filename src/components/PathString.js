...
export default function PathString({
  ...
  useEffect(() => {
    ...
    segments.each(function (d) {
      const currentClass =
        getLevelFromMapList(d) === "m" ? "pathString m" : "pathString b";
      select(this).select("span.pathString").attr("class", currentClass);
    });

    // Handle enters
    const enteredSegments = segments
      .enter()
      .append("div")
      .attr("class", "segment")
      .style("opacity", 0);
